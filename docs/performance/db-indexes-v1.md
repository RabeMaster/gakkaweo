...
>   work_mem은 PostgreSQL이 정렬이나 해시 조인을 메모리에서 처리할 때 쓰는 임시 공간이다.
>   인덱스 스캔이 반환하는 행 위치를 비트맵으로 담는데, 이 비트맵이 너무 커서 work_mem을 넘으면 "lossy" 모드로 바뀐다.
>   이때는 정확한 행 위치 대신 대략적인 페이지 위치만 저장한다.
>   그러면 나중에 테이블을 읽을 때 해당 페이지의 행을 전부 다시 검사해야 하므로 성능이 크게 떨어진다.
>
...
- `pg_stat_statements` 활성화 - 운영에서 실제로 어떤 쿼리가 느린지 수집
  ...
  > 실행된 SQL 쿼리의 통계를 수집하는 PostgreSQL 확장 모듈이다.
  > 쿼리별로 실행 횟수, 총 실행 시간, 평균 실행 시간, 최대/최소 실행 시간을 기록한다.
  > 어떤 쿼리가 자주 실행되고 오래 걸리는지 보이므로, 인덱스 최적화나 쿼리 리팩토링 같은 성능 개선의 출발점이 된다.
- 대용량 테이블에서 락 없이 인덱스 만들기 (Flyway out-of-order + CONCURRENTLY)
- 미사용 인덱스 모니터링 (`pg_stat_user_indexes`)
  > **pg_stat_user_indexes란 무엇인가**
  > 사용자 테이블에 있는 인덱스의 사용 통계를 보여주는 PostgreSQL 시스템 뷰다.
  > 인덱스별로 스캔 횟수, 튜플 읽기 수, 튜플 반환 수를 기록해, 그 인덱스가 쿼리에서 실제로 얼마나 쓰이는지 드러낸다.
  > 그래서 안 쓰이는 인덱스를 찾아 지우거나, 필요한 인덱스가 제대로 쓰이는지 모니터링하는 데 쓴다.
