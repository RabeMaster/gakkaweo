...
> 프론트 경험이 없는 백엔드 개발자보다는, 경험이 있는 백엔드 개발자가 프론트엔드의 주요 패턴과 구조를 어느정도 이해하고 있는 편이 협업에 도움이 될 것 같아서 나름 노력해 봤다.
...
> 예전에 트리쉐이킹을 믿고 배럴 패턴을 남발했지만, 실제로는 트리쉐이킹이 제대로 동작하지 않아 사용하지 않는 코드까지 번들에 들어가는 문제가 있었다.

> 또 특정 타입을 추적하다 보면 배럴 탓에 의존성이 어디서 어디까지 이어지는지 파악하기 어려운 경우가 많았다. 그래서 파일을 직접 import하는 방식으로 바꿨다. 파일이 많아지는 단점은 있지만 의존성 추적은 훨씬 명확해졌다.
...
> Tailwind의 색상 시스템은 고정된 팔레트라서, 유사도에 따라 연속적으로 변하는 색을 표현하기에는 맞지 않는다. 색이 정확히 이어지게 하려면 inline style로 HSL hue를 직접 계산해 적용하는 편이 낫다고 판단했다.
...
> 사용자가 실수로 같은 추측을 여러 번 제출하는 걸 막으려고, `handleSubmit` 함수에서 `displayGuesses` 상태를 먼저 검사해 이미 추측한 문장인지 확인한다. 이미 추측한 문장이면 서버에 중복 요청을 보내지 않고 기존에 계산된 유사도를 바로 보여준다. 불필요한 서버 부하도 줄고 사용자 경험도 나아진다.
...
> 추측을 제출하기 전에 `normalizeGuessText()` 함수로 입력 텍스트를 정규화한다. 이 함수는 백엔드 `TextNormalizer`와 같은 로직으로 특수문자를 지우고 공백을 정규화한다. 덕분에 서버에 불필요한 요청을 보내지 않고도, 입력이 이미 유효하지 않은지 바로 피드백할 수 있다.
