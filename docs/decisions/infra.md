...
Redis는 백업하지 않는다. 랭킹 등 캐시 데이터는 자정 스케줄러가 DB에서 재생성할 수 있는 유실 허용 데이터다. Prometheus/Grafana 관측 데이터도 제외했다. 대시보드는 코드로 관리되어 언제든 다시 만들 수 있다.
...
- 저장 10GB와 전송(egress)이 무료라 비용이 들지 않는다
...
- 버킷 전체에 30일 bucket lock(WORM): 올라간 파일은 30일 동안 누구도 지우거나 덮어쓸 수 없다
