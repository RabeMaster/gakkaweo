...
1. [핵심 원칙](#1-core-principles-핵심-원칙)
2. [Tailwind Config](#2-tailwind-config-필수-확장-설정)
3. [상호작용](#3-interactive-states-상호작용)
4. [색상 규정](#4-color-palette-색상-규정)
5. [다크모드](#5-dark-mode-다크모드)
6. [타이포그래피](#6-typography-타이포그래피)
...
| `DefaultAvatar`   | `shared/ui/DefaultAvatar.tsx`     | 기본 프로필 아바타. `size?: "sm" \| "md" \| "lg"` (default `"sm"`)   |
| `ConfirmDialog`   | `shared/ui/ConfirmDialog.tsx`     | 확인/취소 다이얼로그. Dialog 래핑, `whitespace-pre-line` 메시지 지원 |
| `useEscapeStack`  | `shared/hooks/useEscapeStack.ts`  | Escape 키 스택. Dialog/MobileSideSheet 공유, 최상위만 처리           |
| `useMediaQuery`   | `shared/hooks/useMediaQuery.ts`   | `useSyncExternalStore` 기반 미디어 쿼리. CSR SPA 전용                |
| `useNoIndex`      | `shared/hooks/useNoIndex.ts`      | `<meta name="robots" content="noindex,nofollow">` 동적 삽입/제거     |
| `DesktopOnlyGate` | `shared/ui/DesktopOnlyGate.tsx`   | JS 기반 데스크톱 전용 게이트. 모바일 시 children 미마운트            |
...
공용 컴포넌트 `shared/ui/Dialog.tsx`. 모든 다이얼로그/모달은 이 컴포넌트를 사용한다. `createPortal(document.body)`로 렌더링되어 부모 CSS 간섭 없음.
...
**기능**: Escape 키 (`useEscapeStack` - 최상위만 처리, `disableClose` 시 no-op), click-outside (백드롭 `onMouseDown`), body scroll lock (모듈 레벨 카운터 + scrollbar width `paddingRight` 보상), `role="dialog" aria-modal="true" aria-labelledby`
